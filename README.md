...
The Send-to-Audio handler and extractor live in `src/ingest/url.ts`. They accept approved-user
article requests, extract plain-text content, and hand jobs to an injected queue; they do not
synthesize audio or start a server themselves. See
[the ingest contract, limits, and local acceptance harness](docs/ingest.md). Deno Deploy behavior is
not yet verified.
